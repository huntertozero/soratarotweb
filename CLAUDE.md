...
- **켈틱 크로스 카드 시인성 개선**: READING 화면에서 카드 순번 명확화
  ...
  **해결 방안:**
  - CARD_REVEAL 화면: 현재 상태 유지 (1장 또는 3장은 순번이 명확)
  - READING 화면 (켈틱 크로스만):
    - 좌측 카드 영역: 각 카드 상단에 **번호 뱃지** 추가
      - 디자인: 원형 또는 사각형 뱃지, 숫자만 표시 (예: `1`, `2`, ... `10`)
      - 위치: 카드 이름 위 또는 카드 이미지 좌상단
      - 색상: 카드 색상(--color-starlight)과 조화, 높은 가시성
    - 우측 카드 영역: 좌측과 동일하게 번호 뱃지 추가
    - 중앙 해석: 현재 상태 유지 (텍스트는 "1번 카드는..." 형식으로 통일 가능하나, 필수 아님)
