...
- **24시간 사용 제한**: 각 스프레드 옵션마다 24시간마다 1회 사용 제한
  - 스프레드별 마지막 사용 시간을 localStorage에 저장
  - 사용 후 24시간 카운트다운 타이머 표시
  - UI: 사용 가능할 때까지의 남은 시간 표시 (예: "12시간 35분 후 사용 가능")

- **켈틱 크로스 카드 시인성 개선**: READING 화면에서 카드 구분 강화
  - 좌우 카드 목록에 각 카드의 순번(1~10) 표시 필요
  - 문제: 해석 텍스트에서 "첫 번째 카드는..." 또는 "The Fool은..." 혼용 → 혼동 가능
  - 해결: 카드 옆에 번호 뱃지 추가 (예: `1️⃣ The Fool`, `2️⃣ The Magician`)

- **모바일 최적화**: PC 중심 UI → 모바일 반응형 구성
  - 현재 상태: READING 화면이 좌우 3열(좌측 카드 | 중앙 해석 | 우측 카드) 레이아웃
  - 모바일 예상 사용자: 대다수 (휴대폰 기반 타로 리딩)
  - 개선 필요:
    - 스택 레이아웃 (세로 배치): 카드 → 해석 (모바일)
    - PC에서는 기존 3열 유지
    - 터치 이벤트: 카드 좌우 스와이프로 카드 전환 가능 (선택사항)
    - 폰트 크기, 버튼 크기 모바일 최적화
